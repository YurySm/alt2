document.addEventListener('DOMContentLoaded', () => {
    ...
    btnsFaq.forEach((btn, i)=> {
        ...
        function getCheck() {
            try {
                const check = document.querySelector('input#policy:checked');
                if (check) {
                    document.querySelector('span.policy').style.cssText = `
                       border:1px solid #484ce6;
                       background:#484ce6 url(${templateUrl}/assets/img/contacts/checkbox.svg) center center no-repeat;
                    `;
                } else {
                    document.querySelector('span.policy').style.cssText = ``;
                }
        ...
        try {
            document.querySelector('label[for=policy]').addEventListener('click', () => {
                getCheck();
            });
